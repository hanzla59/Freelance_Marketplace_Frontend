...
const OrderDetailDialog = ({ open, onClose, orderId, price, work, image, video }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md" sx={{ maxWidth: 600, margin: 'auto' }}>
      <DialogTitle>
        <Box display="flex" justifyContent="space-between">
          <Typography variant="h6">Order ID: {orderId}</Typography>
          <Typography variant="h6">{price} PKR</Typography>
      ...
      <DialogActions>
        <Button onClick={onClose} variant="outlined" color="black" fullWidth={isMobile}>
          Close
        </Button>
      </DialogActions>
    </Dialog>
