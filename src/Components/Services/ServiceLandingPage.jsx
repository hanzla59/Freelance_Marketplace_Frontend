...
//   const [showingImages, setShowingImages] = useState(true); // State to toggle between images and video
...
//       } else {
//         // If images are finished, show video
        ...
        <Box sx={{ p: 3 }}>
          ...
          {serviceReviews.length === 0 ? (
            ...
            serviceReviews.map((review, index) => (
              <Card key={index} sx={{ p: 2, mt: 2 }}>
                <Typography>{review.review}</Typography> {/* Changed from `review.comment` */}
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
                  <Typography>{review.buyer.username}</Typography> {/* Changed from `review.username` */}
                  <Rating value={review.rating} readOnly />
                </Box>
              </Card>
            ))
